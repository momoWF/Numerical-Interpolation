...
void SplineO::setTamanho(double novoTamanho){
	tamanho = novoTamanho;
}

int SplineO::getTamanho(){
	return tamanho;
}

void SplineO::calcularG(){
	int i,c,n=g.size();
	double p,un,qn,sig;
	vector<double> aux(n-1);
	g[0]=aux[0]=0.0;			//	Parâmetro do SplineO Cúbico Natural
...
double SplineO::interpolar(double x) {
	...
	a=(vetorX[kMaior]-x)/h;
	b=(x-vetorX[kMenor])/h; // Calculando o SplineO
	y=a*vetorY[kMenor]+b*vetorY[kMaior]+((a*a*a-a)*g[kMenor]+(b*b*b-b)*g[kMaior])*(h*h)/6.0;
	return y;
}

int SplineO::localizar(const double x){
	int jSuperior;
	int jInferior;
	int jm;
	if (getTamanho() < 2){
